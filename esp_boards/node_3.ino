...
void present_initial_values() {
  ...
  sprintf(topicBuff, "%s/3/1/1/0/2", PUBLISH_TOPIC_PREFIX);
  sprintf(msgBuff, "%d", digitalRead(buzzerPin));
  client.publish(topicBuff, msgBuff);

  sprintf(topicBuff, "%s/3/2/1/0/16", PUBLISH_TOPIC_PREFIX);
  sprintf(msgBuff, "%d", digitalRead(motionSensorPin));
  client.publish(topicBuff, msgBuff);

  sprintf(topicBuff, "%s/3/3/1/0/13", PUBLISH_TOPIC_PREFIX);
...
void presentation() {
  ...
  client.publish(topicBuff, "Light sensor");
  sprintf(topicBuff, "%s/3/1/0/0/23", PUBLISH_TOPIC_PREFIX);
  client.publish(topicBuff, "Buzzer");
  sprintf(topicBuff, "%s/3/2/0/0/1", PUBLISH_TOPIC_PREFIX);
  client.publish(topicBuff, "Motion sensor");
  sprintf(topicBuff, "%s/3/3/0/0/15", PUBLISH_TOPIC_PREFIX);
  client.publish(topicBuff, "Distance sensor");
...
void receiveMessage(String topic, byte* payload, unsigned int length) {
  ...
  if (topic == "home-1-in/3/1/2/0/2") {
    sprintf(topicBuff, "%s/3/1/1/0/2", PUBLISH_TOPIC_PREFIX);
    sprintf(msgBuff, "%d", digitalRead(buzzerPin));
    client.publish(topicBuff, msgBuff);
  }
  if (topic == "home-1-in/3/2/2/0/16") {
    sprintf(topicBuff, "%s/3/2/1/0/16", PUBLISH_TOPIC_PREFIX);
    sprintf(msgBuff, "%d", digitalRead(motionSensorPin));
    client.publish(topicBuff, msgBuff);
  }
  if (topic == "home-1-in/3/3/2/0/13") {
    sprintf(topicBuff, "%s/3/3/1/0/13", PUBLISH_TOPIC_PREFIX);
    sprintf(msgBuff, "%d", distanceSensorLastDistance);
    client.publish(topicBuff, msgBuff);
  }
  ...
  if (topic == "home-1-in/3/2/3/0/18") {
    sprintf(topicBuff, "%s/3/2/3/0/22", PUBLISH_TOPIC_PREFIX);
    client.publish(topicBuff, "");
  }
  if (topic == "home-1-in/3/3/3/0/18") {
    sprintf(topicBuff, "%s/3/3/3/0/22", PUBLISH_TOPIC_PREFIX);
    client.publish(topicBuff, "");
  }
}
